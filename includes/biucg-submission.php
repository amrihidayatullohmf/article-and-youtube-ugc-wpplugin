...
<div class="wrap">
	...
	$totalqueue = $table->get_number(array('state'=>2));
	$totalapprove = $table->get_number(array('state'=>1));
	...
	<div class="header-area-table">
		<div class="category-area">
			<a <?php if($status == 'all') echo 'class="active"'; ?> href="<?php echo site_url(); ?>/wp-admin/admin.php?page=biucg-submission&status=all">All (<?php echo ($totalqueue+$totalapprove); ?>)</a>
			&nbsp;|&nbsp;
			<a <?php if($status == 1) echo 'class="active"'; ?> href="<?php echo site_url(); ?>/wp-admin/admin.php?page=biucg-submission&status=1">Approved List (<?php echo $totalapprove; ?>)</a>
			&nbsp;|&nbsp;
			<a <?php if($status == 2) echo 'class="active"'; ?>  href="<?php echo site_url(); ?>/wp-admin/admin.php?page=biucg-submission&status=2">Pending List (<?php echo $totalqueue; ?>)</a>
		</div>
